...
<h4 class='text-end'> &nbsp; Filter Search: </h4>
  ...
  // connect to database
    ...
    WHERE (Art_Name LIKE '%" . $search . "%'
       ...
       OR  Art_Type LIKE '%" . $search . "%'
       OR  Venue LIKE '%" . $search . "%'
       OR  Date_Release LIKE '%" . $search . "%'
       OR  Auction_Date LIKE '%" . $search . "%'
       OR  Bid_Time LIKE '%" . $search . "%'
       OR  Art_Bid LIKE '%" . $search . "%'
...
<?php
   if ($foundnum==0)
   {
     echo "<h1 class='text-center'><br><br><br><strong>We were unable to find an Art with a search term of '<b> $search</b>'.</strong></h1>";
   }
   else{
     echo "<br><br><br><h1 class='text-center'><strong> $foundnum Results Found for \"" .$search."\" </strong></h1>";      
